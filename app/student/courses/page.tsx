import { StudentCoursesList } from '@/components/student/student-dashboard'
import { createClient } from '@/lib/supabase/server'
import { requireStudent } from '@/lib/auth/guards'
import { loadStripeIntegration } from '@/lib/integrations/stripe'
import { loadEnrollmentSettings } from '@/lib/enrollment/service'
import { createServiceClient } from '@/lib/supabase/service'
import type { Course, Enrollment } from '@/lib/types/database'

export default async function Page() {
  const { profile } = await requireStudent()
  const supabase = await createClient()
  const service = createServiceClient()
  const [{ data: enrollments }, { data: courses }, stripe, enrollment] = await Promise.all([
    supabase.from('enrollments').select('*').eq('user_id', profile.id),
    supabase.from('courses').select('*').eq('published', true).order('sort_order'),
    loadStripeIntegration(),
    loadEnrollmentSettings(service),
  ])
  const prog = Object.fromEntries(((enrollments ?? []) as Enrollment[]).map((e) => [e.course_id, e.progress_pct]))
  const enrolledCourses = ((courses ?? []) as Course[])
    .filter((c) => prog[c.id] !== undefined)
    .map((c) => ({ ...c, progress_pct: prog[c.id] ?? 0 }))
  const availableCourses = ((courses ?? []) as Course[]).filter((c) => prog[c.id] === undefined)

  return (
    <StudentCoursesList
      profile={profile}
      initialCourses={enrolledCourses}
      availableCourses={availableCourses}
      stripeEnabled={Boolean(stripe?.enabled && stripe.secretKey && stripe.publishableKey)}
      academyPriceConfigured={Boolean(enrollment.stripePriceId)}
    />
  )
}
